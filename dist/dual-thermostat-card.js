...
class DualThermostatCard extends LitElement {
  ...
  constructor() {
      ...
      this._cachedMods = null;
      this._lastEntity1 = undefined;
      this._lastEntity2 = undefined;
  }
  ...
  setConfig(config) {
    ...
    if (!this._cachedMods || e1 !== this._lastEntity1 || e2 !== this._lastEntity2) {
  ...
  _buildCards() {
    ...
    this._offscreen.replaceChildren(); // очищаем старые карточки перед добавлением новых
    ...
    return new Promise((resolve) => {
      const check = () => {
        ...
        for (const s of styles) {
          if (s.textContent?.includes("#1C1B1F")) return true;
        }
        return false;
      };
      ...
      const mo = new MutationObserver(() => {
        if (check()) { mo.disconnect(); resolve(); }
      });

      if (card.shadowRoot) {
        mo.observe(card.shadowRoot, { childList: true, subtree: true, characterData: true });
      }

      setTimeout(() => { mo.disconnect(); resolve(); }, 1000);
    });
  }
  ...
  firstUpdated() {
    ...
    if (frame) {
      ...
      frame.addEventListener("pointerdown", this._onPointerDownBound);
      frame.addEventListener("pointerup", this._onPointerUpBound);
      frame.addEventListener("click", this._onClickBound);
    }
  }
  disconnectedCallback() {
    ...
    const frame = this.shadowRoot?.querySelector(".card");
    if (frame) {
      if (this._onPointerDownBound) frame.removeEventListener("pointerdown", this._onPointerDownBound);
      if (this._onPointerUpBound) frame.removeEventListener("pointerup", this._onPointerUpBound);
      if (this._onClickBound) frame.removeEventListener("click", this._onClickBound);
    }
  }
