...
class DualThermostatCard extends LitElement {
  ...
  _waitForCardModReady(card) {
    return new Promise((resolve) => {
      ...
      const checkStyles = (shadow) => {
        if (!shadow) return false;
        for (const s of shadow.querySelectorAll("style")) {
          if (s.textContent?.includes(MARKER)) return true;
        }
        return false;
      };
      ...
      const waitForShadow = () => {
        if (!card.shadowRoot) {
          setTimeout(waitForShadow, 20);
          return;
        }

        if (checkStyles(card.shadowRoot)) {
          resolve();
          return;
        }
